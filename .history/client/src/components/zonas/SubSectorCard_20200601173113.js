...
const SubSectorCard = ({ sb }) => {
  ...
  if (contadores) {
    ...
    const id = contadores.map((c) => c.subsector._id);
    console.log(id);
    console.log(id[0] === sb._id);
    const contador = contadores.filter((cont) => cont.subsector._id === sb._id);
    console.log(contador[0].counter);
  }
  return (
    <div>
      <div
