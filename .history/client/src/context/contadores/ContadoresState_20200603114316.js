...
const ContadoresState = (props) => {
  ...
  const getContadorSector = async (zonas) => {
    try {
      ...
      await sectoresId.map((id) =>
        res.push(axios.get(`/api/v1/sectores/${id}/contadores`))
      );

      console.log(res);

      let cS = res.map((r) => console.log(r.data.data));

      const contadorSector = cS;
      console.log(contadorSector);

      dispatch({ type: GET_CONTADORES_SECTOR, payload: {} });
    } catch (err) {
      dispatch({ type: ERROR_CONTADORES, payload: err.response });
    }
  };
