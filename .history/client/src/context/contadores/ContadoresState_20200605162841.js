...
const ContadoresState = (props) => {
  ...
  const addContador = async (contador, contadorId, sectorId) => {
    ...
    if (!contadorId) {
      try {
        ...
        if (
          state.contadorSector.filter(
            (cS) => sameDay(cS.fecha, state.thisFecha) && cS.sector === sectorId
          ).length === 0
        ) {
          console.log('No hay contGlobal aun');
          getContadorSector(state.zonas);
        }
  ...
  const addContadorSector = (contador, sectorId) => {
    contadorSector = {
      _id: { sector: sectorId },
      fecha: thisFecha,
      contador,
