...
exports.getContadores = asyncHandler(async (req, res, next) => {
  if (req.params.subsectorId) {
    ...
    console.log(typeof req.params.sectorId);

    const sectorId = mongoose.Types.ObjectId(req.params.sectorId);
    console.log(typeof sectorId);
    ...
    const sumaContador = await Contador.aggregate([
      ...
      // { $match: { 'subsectores.sector': req.params.sectorId } },
    ]);

    let a = sumaContador.map((b) =>
      console.log(b.subsectores.sector === sectorId)
    );
  } else {
    res.status(200).json({
      ...res.advancedResults,
