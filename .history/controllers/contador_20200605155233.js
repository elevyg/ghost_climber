...
exports.getContadores = asyncHandler(async (req, res, next) => {
  ...
  // Contadores acumulados de un sector
    ...
    sumaContador.map((c) => {
      c.fecha = new Date(c._id.year, c._id.month - 1, c._id.day);
    });

    delete sumaContador._id.day;
    delete sumaContador._id.month;
    delete sumaContador._id.year;
...
exports.crearContador = asyncHandler(async (req, res, next) => {
  ...
  console.log(sb_contador.length);
  const sb_date = sb_contador.filter((sb) => sameDay(sb.fecha, req.body.fecha));

  console.log('Fecha en db: ' + sb_date.length);
  console.log('Fecha en req: ' + req.body.fecha);
