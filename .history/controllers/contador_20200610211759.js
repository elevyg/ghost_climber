...
exports.getContadores = asyncHandler(async (req, res, next) => {
  ...
  // Contadores acumulados de un sector
    ...
    sumaContador.map((c) => {
      c.fecha = moment(new Date(c._id.year, c._id.month - 1, c._id.day)).tz(
        'America/Santiago'
      );
      delete c._id.day;
      delete c._id.month;
...
exports.crearContador = asyncHandler(async (req, res, next) => {
  ...
  const sb_date = sb_contador.filter((sb) =>
    moment(sb.fecha).isSame(req.body.fecha, 'day')
  );
  ...
  if (sb_date.length > 0) {
    ...
    res.status(200).json({
      success: true,
      data: contador,
      msg: `Contador con Id ${req.params.id} actualizado en vez de creado.`,
    });
  } else {
